import { Character, COMBAT_EVENTS, TurnOwner, TurnStartedEvent, CombatEndedEvent, CombatState, DamageDealtEvent, AttackCommand, PRESENTATION_EVENTS, PresentationCompleteEvent, CombatStartedEvent, CharacterDiedEvent } from "./combat-types.js";
import { CombatSystem } from "./combat-system.js";
import { PresentationController } from "./presentation.js";
import { SimpleRenderer } from './renderer.js';

const player1: Character = new Character("Marcus", 100, 12, 3, 5);
const player2: Character = new Character("Amy", 80, 15, 1, 10);
const enemy1: Character = new Character("Goblin", 50, 5, 1, 8);
const enemy2: Character = new Character("Orc", 80, 12, 3, 6);
const enemy3: Character = new Character("Imp", 60, 8, 0, 15);

const playerParty: Character[] = [player1, player2];
const enemyParty: Character[] = [enemy1, enemy2, enemy3];
...
const targetSelection = document.getElementById("target-selection") as HTMLDivElement;

if (!attackBtn || !targetSelection) {
    throw new Error('Could not find required HTML elements');
}

combatSystem.on(COMBAT_EVENTS.TURN_STARTED, (data: TurnStartedEvent) => {
...
combatSystem.on(COMBAT_EVENTS.COMBAT_ENDED, (data: CombatEndedEvent) => {
...
combatSystem.on(COMBAT_EVENTS.COMBAT_STARTED, (data: CombatStartedEvent) => {
...
combatSystem.on(COMBAT_EVENTS.CHARACTER_DIED, (data: CharacterDiedEvent) => {
    const currentState = combatSystem.getCurrentState();
    if (currentState === CombatState.ProcessingPlayerAction) {
...
presentationController.on(PRESENTATION_EVENTS.COMPLETE, (data: PresentationCompleteEvent) => {
    const currentState = combatSystem.getCurrentState();
...
combatSystem.on(COMBAT_EVENTS.DAMAGE_DEALT, (data: DamageDealtEvent) => {
    simpleRenderer.draw(data.target, data.amount);
});

attackBtn.addEventListener('click', () => {
    const selectedTarget = document.querySelector<HTMLInputElement>('input[name="enemy"]:checked');
    if (!selectedTarget) {
